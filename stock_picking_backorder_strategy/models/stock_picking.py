...
class StockPicking(models.Model):
    ...
    def action_done(self):
        return super(
            StockPicking,
            self.with_context(on_picking_validation=True)
        ).action_done()
    ...
    def _create_backorder(self, backorder_moves=None):
        ...
        pickings = self.filtered(
            ...
            #or the ones which we only want to skip backorder if we are validating
            #the picking as a whole, but we are not on such context (validating only a
            #stock move, for example).
            lambda p: (p.picking_type_id.backorder_strategy != 'no_create') or\
                (
                    p.picking_type_id.disable_backorder_only_on_picking_validation and\
                    not self._context.get('on_picking_validation')
                )
        )
